...
void performConnection(int sock, opt_t *opt, config_t *config, P_FLAG f) {
  ...
  void *recvBuff;
  recvBuff = recvCommand(sock, 1); // + MNM Gameserver <<Gameserver Version>>
  ...
  recvBuff = recvCommand(
      sock, 1); // + Client version accepted - please send Game-ID to join
  parseCommand(recvBuff, VERSION);
  free(recvBuff);

  sendCommand(sock, ID, opt->gameId);

  recvBuff =
      recvCommand(sock, 2); // + PLAYING <<Gamekind-Name>>\n + <<Game-Name>>
  parseCommand(recvBuff, ID);
  free(recvBuff);

  // retry without player id later
  sendCommand(sock, PLAYER, opt->playerId);

  recvBuff = recvCommand(sock, 0); //
  parseCommand(recvBuff, PLAYER);
  free(recvBuff);

  printProlog(getGameInfo(), f);
}